-- Add the gingerbreadman map type to both configuration table constraints
-- (positioned after arnold-cat to match VALID_MAP_TYPES order)
-- Wrapped in a transaction so a mid-migration failure leaves no partial state.

BEGIN;

-- Update saved_configurations constraint with all 20 map types
...
-- Update shared_configurations constraint with all 20 map types
