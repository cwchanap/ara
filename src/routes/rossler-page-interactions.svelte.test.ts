...
describe('Rossler page interactions', () => {
	beforeEach(() => {
		vi.useFakeTimers();
		setupApiFetchMock();
	});
	...
	it('renders correctly and has title', () => {
		setMockPageUrl('http://localhost/rossler');
		render(RosslerPage, { props: authedPageProps });
		expect(screen.getByText('RÖSSLER_ATTRACTOR')).toBeInTheDocument();
	});
	...
	it('shows save and share dialogs, calls handleSave and handleShare callbacks', async () => {
		...
		const dialogSaveBtn = screen.getByTestId('dialog-save-rossler');
		...
		const dialogShareBtn = screen.getByTestId('dialog-share-rossler');
