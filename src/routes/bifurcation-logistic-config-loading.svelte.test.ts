/**
 * Config-loading tests for the bifurcation-logistic visualization page (uses useConfigLoader hook).
 */
...
const baseParams = { type: 'bifurcation-logistic', rMin: 3.5, rMax: 4.0, maxIterations: 1000 };
...
describe('bifurcation-logistic page – config loading', () => {
	...
	it('loads config from configId and applies parameters', async () => {
		...
		await waitFor(() => {
			const slider = document.getElementById('rMin') as HTMLInputElement;
	...
	it('loads config from share code and applies parameters', async () => {
		loadSharedConfigParametersMock.mockResolvedValueOnce({
			ok: true,
			parameters: { ...baseParams, rMin: 3.8 },
			source: 'sharedApi'
		});
		...
		await waitFor(() => {
			expect(loadSharedConfigParametersMock).toHaveBeenCalledWith(
				expect.objectContaining({
					shareCode: 'bifurcation-logistic-share-1',
					mapType: 'bifurcation-logistic'
				})
			);
		});
	...
	it('shows error when inline config param is invalid', async () => {
		parseConfigParamMock.mockReturnValueOnce({
			...
			errors: ['Bad bifurcation-logistic params'],
			logMessage: 'err',
			logDetails: {}
		});
	...
	it('does not update state when unmounted before async config load resolves', async () => {
		...
		const { unmount } = render(ubifurcationlogisticPage, { props: pageProps });
	...
	it('dismisses save error toast when the dismiss button is clicked', async () => {
		...
		try {
			...
			await fireEvent.click(screen.getByTestId('dialog-save-bifurcation-logistic'));
	...
	it('closes the share dialog via onClose callback', async () => {
		...
		expect(screen.getByTestId('dialog-stub-bifurcation-logistic')).toBeInTheDocument();

		await fireEvent.click(screen.getByTestId('dialog-close-bifurcation-logistic'));
		expect(screen.queryByTestId('dialog-stub-bifurcation-logistic')).not.toBeInTheDocument();
	});
});
