/**
 * Config-loading tests for the henon visualization page (uses useConfigLoader hook).
 */
...
const baseParams = { type: 'henon', a: 1.4, b: 0.3, iterations: 2000 };
...
describe('henon page – config loading', () => {
	...
	it('loads config from configId and applies parameters', async () => {
		loadSavedConfigParametersMock.mockResolvedValueOnce({
			ok: true,
			parameters: { ...baseParams, a: 1.2 },
			source: 'api'
		});

		setPageUrl('http://localhost/henon?configId=henon-id-1');
		render(uhenonPage, { props: pageProps });

		await waitFor(() => {
			expect(loadSavedConfigParametersMock).toHaveBeenCalledWith(
				expect.objectContaining({ configId: 'henon-id-1', mapType: 'henon' })
			);
		});

		await waitFor(() => {
			const slider = document.getElementById('a') as HTMLInputElement;
	...
	it('loads config from share code and applies parameters', async () => {
		...
		await waitFor(() => {
			expect(loadSharedConfigParametersMock).toHaveBeenCalledWith(
				expect.objectContaining({ shareCode: 'henon-share-1', mapType: 'henon' })
			);
		});
	...
	it('applies config from inline config param', async () => {
		...
		await waitFor(() => {
			expect(parseConfigParamMock).toHaveBeenCalledWith(
				expect.objectContaining({ mapType: 'henon', configParam: 'some-encoded-data' })
			);
		});
	...
	it('does not update state when unmounted before async config load resolves', async () => {
		...
		const { unmount } = render(uhenonPage, { props: pageProps });
	...
	it('dismisses save error toast when the dismiss button is clicked', async () => {
		...
		try {
			...
			await fireEvent.click(screen.getByTestId('dialog-save-henon'));
	...
	it('closes the share dialog via onClose callback', async () => {
		...
		expect(screen.getByTestId('dialog-stub-henon')).toBeInTheDocument();

		await fireEvent.click(screen.getByTestId('dialog-close-henon'));
		expect(screen.queryByTestId('dialog-stub-henon')).not.toBeInTheDocument();
	});
});
