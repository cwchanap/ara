<script lang="ts">
	...
	$effect(() => {
		const url = $page.url;
		untrack(() => {
			const incoming = decodeComparisonState(url, 'bakers-map');
			const leftIncoming = clampParams(incoming?.left as BakersMapParameters | null);
			const rightIncoming = clampParams(incoming?.right as BakersMapParameters | null);
