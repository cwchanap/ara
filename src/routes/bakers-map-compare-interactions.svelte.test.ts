/**
 * Compare-page interaction tests for the Baker's Map comparison route.
...
import BakersMapComparePage from './bakers-map/compare/+page.svelte';

const mockGoto = vi.hoisted(() => vi.fn());

const pageStore = vi.hoisted(() => {
	let value: Page = {
		url: new URL('http://localhost/bakers-map/compare?compare=true') as Page['url'],
		params: {},
		route: { id: null },
		status: 200,
		error: null,
...
describe("Baker's Map compare page interactions", () => {
	beforeEach(() => {
		setPageUrl('http://localhost/bakers-map/compare?compare=true');
	});
	...
	it('renders the recurrence equations in both panels', () => {
		render(BakersMapComparePage);
		const equations = screen.getAllByText(/x\(n\+1\) = 2x\(n\) mod 1/i);
		expect(equations.length).toBeGreaterThanOrEqual(2);
	});

	it('displays default pointCount (3000) and speed (1) for both panels', () => {
		const { container } = render(BakersMapComparePage);
	...
	it('renders without compare param (falls back to defaults)', () => {
		setPageUrl('http://localhost/bakers-map/compare');
		const { container } = render(BakersMapComparePage);
		expect(container.querySelector('#left-pointCount')).not.toBeNull();
		expect(container.querySelector('#right-pointCount')).not.toBeNull();
	});
	...
	it('cleans up on unmount without throwing', () => {
		const { unmount } = render(BakersMapComparePage);
