...
function makeResult(points: [number, number, number][], speeds?: number[]): LorenzResult {
...
describe('colors.ts', () => {
	...
	it('clamps interpolation input values in clamp01', () => {
		...
		// but we can pass negative speeds to makeResult to force t < 0.
		const result = makeResult(
			[
				[0, 0, 0],
				[1, 1, 1],
