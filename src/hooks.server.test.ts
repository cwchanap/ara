...
const h = vi.hoisted(() => {
	...
	const authClientFactory = vi.fn(() => ({ getSession }));

	const createNeonAuthClient = vi.fn((options: { headers?: HeadersInit } = {}) =>
	...
	const exchangeOAuthVerifier = vi.fn(async ({ request }: { request: Request }) => {
	...
	const fetchServerSession = vi.fn(async (request: Request) => {
		void request;
		return state.fetchSessionResult;
	});
...
const handle = createHandle({
	createNeonAuthClient: h.createNeonAuthClient,
	exchangeOAuthVerifier: h.exchangeOAuthVerifier,
	fetchServerSession: h.fetchServerSession
});

describe('hooks.server', () => {
	beforeEach(() => {
		h.state.fetchSessionResult = {
			data: { user: { id: 'user-1', email: 'ada@example.com', name: 'Ada' } },
			ok: true
		};
		h.getSession.mockClear();
		h.createNeonAuthClient.mockClear();
		h.authClientFactory.mockClear();
		h.exchangeOAuthVerifier.mockClear();
		h.fetchServerSession.mockClear();
	});
	...
	test('exchanges OAuth verifier before resolving protected routes', async () => {
		...
		expect(h.exchangeOAuthVerifier).toHaveBeenCalledWith({ request });
	...
	test('continues normal setup when no OAuth verifier is present', async () => {
		...
		expect(h.exchangeOAuthVerifier).not.toHaveBeenCalled();
		expect(h.authClientFactory).toHaveBeenCalledWith('https://auth.example.test/auth', {
			fetchOptions: {
				headers: undefined
			}
		});
	...
	test('sets neonAuth and safeGetSession on locals', async () => {
		...
		expect(h.fetchServerSession).toHaveBeenCalledWith(request);
	...
	test('safeGetSession returns null session and user when Neon Auth returns an error', async () => {
		h.state.fetchSessionResult = { data: null, ok: false };
		const request = new Request('http://localhost', {
	...
	test('safeGetSession returns null when direct fetch throws', async () => {
		h.fetchServerSession.mockImplementationOnce(async () => {
	...
	test('failed OAuth exchange logs error and continues to normal setup', async () => {
		h.exchangeOAuthVerifier.mockImplementationOnce(async () => ({
			ok: false,
			status: 500,
